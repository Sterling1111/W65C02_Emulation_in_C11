...
byte busy;
uint8_t data_lines = 0;
byte RW = 0, RS = 0;
uint64_t lcdInstructionStartTimePoint = 0;
uint64_t lcdFunctionDuration = 0;
...
void sendCommand(VrEmuLcd* lcd, uint8_t data) {
    busy = 1;
    lcdInstructionStartTimePoint = __builtin_ia32_rdtsc();
    vrEmuLcdSendCommand(lcd, data);
}

void writeByteToLcd(VrEmuLcd* lcd, uint8_t data) {
    busy = 1;
    lcdInstructionStartTimePoint = __builtin_ia32_rdtsc();
    vrEmuLcdWriteByte(lcd, data);
}

void portAWrite(VrEmuLcd* lcd, byte data) {
    RW = data & 0b01000000;
    RS = data & 0b00100000;
    byte line = data & 0b10000000;
    if(!E.enable_line && line) E.toggled_on = 1;
    else E.toggled_on = 0;
    E.enable_line = line;
    if(E.toggled_on && !busy) {
        if(!RS && !RW) {
            sendCommand(lcd, data_lines);
        } else if(!RS) {
            data_lines = 0x00;
        } else if(!RW) {
            writeByteToLcd(lcd, data_lines);
        } else {
            //unknown behavior
        }
